		...
		if (idThread % 2 == 1) {
			...
			while (tempoNoFogo < tempoCozimento) {
				try {
					...
					if (percentCozimento >= 1) {
						System.out.println("Sopa de Cebola #"+idThread+": % cozimento = 100% \nThread #"+idThread+": Sopa de Cebola cozida!");
					} else {
						System.out.println("Sopa de Cebola #"+idThread+": % cozimento = "+(int) (percentCozimento*100)+"%");
					}
			...
			while (tempoNoFogo < tempoCozimento) {
				try {
					...
					percentCozimento = (tempoNoFogo / tempoCozimento);
					if (percentCozimento >= 1) {
						System.out.println("Lasanha a Bolonhesa #"+idThread+": % cozimento = 100% \nThread #"+idThread+": Lasanha a Bolonhesa cozida!");
					} else {
						System.out.println("Lasanha a Bolonhesa #"+idThread+": % cozimento = "+(int) (percentCozimento*100)+"%");
					}
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
