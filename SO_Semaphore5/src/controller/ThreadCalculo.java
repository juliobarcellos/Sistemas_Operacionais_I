...
public class ThreadCalculo extends Thread{
	...
	public ThreadCalculo(int id, Semaphore semaforoBD) {
		this.idThread = id;
	...
	private void Calcula() {
		try {
			...
			System.out.format("A Thread "+idThread+" calculou por %.f segundos. \n", tempo);
	...
	private void Grava() {
		try {
			semaforoBD.acquire();
			sleep((long)tempoBD);
			System.out.println("A Thread "+idThread+" gravou no BD por "+(double)(tempoBD/1000)+" segundos.");
		} catch (InterruptedException e) {
			e.printStackTrace();
		} finally {
			semaforoBD.release();
